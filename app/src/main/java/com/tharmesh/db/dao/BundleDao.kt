...
interface BundleDao {
    ...
    fun getByBundleId(bundleId: String): BundleEntity?

    @Query("SELECT * FROM bundles WHERE status != 'EXPIRED' AND status != 'DELIVERED' AND nextRetryAt <= :nowMs ORDER BY createdAt DESC")
    ...
    @Query("UPDATE bundles SET status = 'RELAYED' WHERE bundleId = :bundleId AND status != 'DELIVERED'")
    fun markRelayed(bundleId: String): Int

    @Query("UPDATE bundles SET status = 'DELIVERED' WHERE bundleId = :bundleId")
    fun markDelivered(bundleId: String): Int

    @Query("UPDATE bundles SET attemptCount = attemptCount + 1, lastAttemptAt = :attemptAt, nextRetryAt = :nextRetryAt WHERE bundleId = :bundleId")
    fun bumpAttemptAndSchedule(bundleId: String, attemptAt: Long, nextRetryAt: Long): Int
