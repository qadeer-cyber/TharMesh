...
object CryptoBox {
    ...
    fun sealForReceiver(
        ...
        if (receiverPublicKeyBase64.isBlank() || senderPrivateKeyBase64.isBlank()) {
            throw IllegalArgumentException("Missing sender/receiver key material")
        }
    ...
    fun openFromSender(
        ...
        return try {
            ...
            val version = json.optInt("v", ENVELOPE_VERSION)
    ...
    private fun aesEncrypt(plainText: String, key: SecretKey, iv: ByteArray): String {
        ...
        val out = cipher.doFinal(plainText.toByteArray(Charset.forName(UTF8)))
    ...
    private fun aesDecrypt(cipherTextBase64: String, key: SecretKey, iv: ByteArray): String {
        ...
        return String(cipher.doFinal(bytes), Charset.forName(UTF8))
    }

    private fun rsaEncrypt(value: String, publicKey: PublicKey): String {
        val cipher = Cipher.getInstance("RSA/ECB/PKCS1Padding")
        cipher.init(Cipher.ENCRYPT_MODE, publicKey)
        val out = cipher.doFinal(value.toByteArray(Charset.forName(UTF8)))
        return Base64.encodeToString(out, Base64.NO_WRAP)
    }
    ...
    private fun rsaDecrypt(valueBase64: String, privateKey: PrivateKey): String {
        ...
        return String(out, Charset.forName(UTF8))
    }

    private fun rsaSign(value: String, privateKey: PrivateKey): String {
        val signature = Signature.getInstance("SHA256withRSA")
        signature.initSign(privateKey)
        signature.update(value.toByteArray(Charset.forName(UTF8)))
    ...
    fun sha256(value: String): String {
        val digest = MessageDigest.getInstance("SHA-256").digest(value.toByteArray(Charset.forName(UTF8)))
