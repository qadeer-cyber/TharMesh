...
class ContactsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        myQrButton.setOnClickListener { startActivity(Intent(this, MyQrActivity::class.java)) }
        ...
        inviteButton.setOnClickListener { showAddInviteDialog() }
        ...
        val bottomNav = BottomNavigationView(this).apply {
            ...
            selectedItemId = R.id.nav_contacts
            setOnItemSelectedListener { item ->
                if (item.itemId == selectedItemId) return@setOnItemSelectedListener true
                when (item.itemId) {
                    R.id.nav_chats -> {
                        startActivity(Intent(this@ContactsActivity, ChatsActivity::class.java))
                        finish()
                        true
                    }
                    R.id.nav_status -> {
                        startActivity(Intent(this@ContactsActivity, StatusActivity::class.java))
                        finish()
                        true
                    }
                    R.id.nav_groups -> {
                        startActivity(Intent(this@ContactsActivity, GroupsActivity::class.java))
                        finish()
                        true
                    }
                    else -> false
    ...
    private fun showAddInviteDialog() {
        ...
        AlertDialog.Builder(this)
            ...
            .setPositiveButton("Add") { _, _ ->
                ...
                if (code.isNotEmpty()) addContact(code)
            }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == REQUEST_SCAN_QR && resultCode == Activity.RESULT_OK) {
            ...
            if (userId.isNotEmpty()) addContact(userId, name, publicKey)
        }
    }
